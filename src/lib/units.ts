...
/** Convierte un peso canónico (kg, tal cual se guarda en la base) a la
...
/** Convierte un peso ingresado por el usuario (en su unidad preferida)
 *  al valor canónico en kg que se guarda en la base de datos. */
export function toCanonicalKg(displayWeight: number, unit: WeightUnit): number {
  return unit === "LB" ? lbToKg(displayWeight) : displayWeight;
}
