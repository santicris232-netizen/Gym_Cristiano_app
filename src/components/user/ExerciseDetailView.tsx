...
export function ExerciseDetailView({
  ...
  const trainHref = `/dashboard/exercise/${exercise.id}/train?sets=${sets}&reps=${reps}${
      ...
      <div className="overflow-hidden rounded-xl border border-line bg-surface">
        ...
        <div className="p-5">
          <div className="mb-2 flex flex-wrap gap-1.5">
            <Badge tone="blue">{bodyPartLabel(exercise.bodyPart)}</Badge>
          ...
          <ol className="mb-4 list-decimal space-y-1.5 pl-5 text-sm text-cream">
            {exercise.instructionStepsEs.map((step, i) => (
              <li key={i}>{step}</li>
            ))}
          </ol>
          ...
          {exercise.secondaryMuscles.length > 0 && (
            <p className="mb-5 text-xs text-cream-dim">
              ...
              {exercise.secondaryMuscles.map(bodyPartLabel).join(", ")}
            </p>
          )}

          <LinkButton href={trainHref} className="mb-4 w-full">
