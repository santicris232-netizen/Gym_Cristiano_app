...
import type { ExerciseSummary, ProgressPointDTO } from "@/types";
...
export function ProgressChart({
  ...
  useEffect(() => {
    ...
    let active = true;
    fetch(`/api/users/${userId}/progress/${exerciseId}`)
      .then((r) => r.json())
      .then((data) => {
  ...
  const chart = useMemo(() => {
    ...
    const weights = points.map((p) => displayWeight(p.maxWeightKg, weightUnit));
    ...
    const coords = points.map((p, i) => {
      ...
      const w = displayWeight(p.maxWeightKg, weightUnit);
  ...
  if (exercises.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-line p-6 text-center text-sm text-cream-dim">
        Este alumno todavía no tiene ejercicios asignados en su plan.
      </p>
    );
  }
            ...
            {chart.coords.map((c, i) => (
              <g key={i}>
                <circle
                  ...
                  strokeWidth={2}
                >
                  <title>{`${formatDateShort(c.point.date)}: ${formatWeight(c.point.maxWeightKg, weightUnit)}`}</title>
                </circle>
                <text
                  x={c.x}
