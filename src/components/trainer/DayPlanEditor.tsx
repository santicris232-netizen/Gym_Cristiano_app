...
import type { PlanEntryDTO, ExerciseSummary } from "@/types";
...
function toDraft(entries: PlanEntryDTO[]): DraftEntry[] {
...
export function DayPlanEditor({
  ...
  userId: string;
  dayOfWeek: number;
  initialEntries: PlanEntryDTO[];
  onSaved: (entries: PlanEntryDTO[]) => void;
}) {
  ...
  async function handleSave() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/users/${userId}/plan/${dayOfWeek}`, {
        method: "PUT",
      ...
      setEntries(toDraft(data.day.entries));
      onSaved(data.day.entries);
      setDirty(false);
    } catch {
  ...
  return (
    <div className="space-y-4">
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <label className="flex flex-col items-center">
                  <span className="font-mono text-[10px] text-cream-dim uppercase">
                    Sets
                  </span>
                  <Input
                    type="number"
      ...
      <div className="flex flex-wrap items-center gap-3 border-t border-line pt-4">
        ...
        {!error && dirty && (
          <span className="text-xs text-cream-dim">Cambios sin guardar</span>
        )}
      </div>
    </div>
  );
